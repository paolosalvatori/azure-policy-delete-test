...
echo "Deleting [$policyDefinitionName] policy definition from the [$subscriptionName] subscription..."
error=$(az policy definition delete --name $policyDefinitionName 2>&1)
if [[ $? == 0 ]]; then
    echo "[$policyDefinitionName] policy definition successfully deleted from the [$subscriptionName] subscription"
else
    echo "Failed to delete [$policyDefinitionName] policy definition from the [$subscriptionName] subscription"
    echo -e "\033[38;5;1m${error}\033[m"
fi